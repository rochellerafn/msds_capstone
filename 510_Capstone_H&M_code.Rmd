...
```{r}
seventeen_thirty8_ta <- seventeen_thirty8_trns %>%
  merge(x=seventeen_thirty8_trns, y=articles, by.x = 'article_id', by.y = 'article_id', all.x = TRUE) %>%
  select(customer_id, age, prod_name, detail_desc, t_dat, Active, fashion_news_frequency, postal_code, article_id, price)

str(seventeen_thirty8_ta)
```

```{r}
head(seventeen_thirty8_ta)
```
...
```{r}
transactions %>%
  mutate(item_count = as.character(count(article_id))) %>%
  group_by(t_dat, customer_id) %>%
  select(t_day, customer_id, item_count)
```
