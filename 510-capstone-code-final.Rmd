...
```{r}
all_sample_transactions <- customer_sample %>%
  left_join(x=customer_sample, y=all_age_transactions, by="customer_id")
...
```{r}
all_sample_transactions <- all_sample_transactions %>%
  left_join(x=all_sample_transactions, y=all_article_words, by="article_id")
...
```{r}
young_sample_trans <- transactions %>%
  inner_join(x=transactions, y=young_sample, by = 'customer_id')
...
```{r}
young_sample_trans <- young_sample_trans %>%
  inner_join(x=young_sample_trans, y=all_article_words, by = 'article_id')
...
```{r}
mature_sample_trans <- transactions %>%
  inner_join(x=transactions, y=mature_sample, by = 'customer_id')

str(mature_sample_trans)
```

```{r}
mature_sample_trans <- mature_sample_trans %>%
  inner_join(x=mature_sample_trans, y=all_article_words, by = 'article_id')
