...
```{r}
all_sample_transactions <- left_join(x=all_sample_transactions, y=all_article_words_u, by="article_id")
...
```{r}
young_sample_words_df$Text <- gsub('[0-9]+', '', young_sample_words_df$Text)

head(young_sample_words_df, 10)
```
...
```{r}
# d <- unlist(strsplit(as.character(young_sample_words_df), split=" "))

# paste(unique(young_sample_words_df), collapse = ' ')
```


```{r}
# rem_dup_word <- function(x){
# paste(unique(trimws(unlist(strsplit(young_sample_words_df,split=" ",fixed=F,perl=T)))),collapse = " ")
# }
```
...
```{r}
young_sample_cor <- Corpus(VectorSource(young_sample_words_df$Text))
```
...
```{r}
mature_sample_words_df$Text <- gsub('[0-9]+', '', mature_sample_words_df$Text)

head(mature_sample_words_df, 10)
```
