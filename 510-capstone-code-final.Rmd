...
```{r}
mature_words_df$Text <- gsub('[0-9]+', '', mature_words_df$Text)

head(mature_words_df, 10)
```
...
```{r}
all_article_words$words <- gsub("[0-9]+", "", all_article_words$words, fixed =TRUE)
...
```{r}
mature_sample_trans <- left_join(x=transactions, y=mature_sample, by = 'customer_id')
