...
```{r}
wordcloud(young_cor_words_u, scale = c(2, 1), min.freq = 500, colors = rainbow(30))
```

```{r}
wordcloud(mature_cor_words_u, scale = c(2, 1), min.freq = 500, colors = rainbow(30))
```
...
```{r}
age_post_lm <- lm(age ~ postal_code, data = all_sample_transactions)

get_regression_table(age_post_lm)
get_regression_summaries(age_post_lm)
```

```{r}
customers <- customers %>%
  mutate(club_member_status = ifelse(club_member_status == "ACTIVE", 1, 0))

customers[["club_member_status"]][is.na(customers[["club_member_status"]])] <- 0
```


```{r}
customer_active_club_lm <- lm(Active ~ club_member_status, data = customers)

get_regression_table(customer_active_club_lm)
get_regression_summaries(customer_active_club_lm)
```

```{r}
ggplot(customers, aes(Active, club_member_status))+
  geom_point()
```
