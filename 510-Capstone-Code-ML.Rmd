...
```{r}
items_per_tran <- left_join(x=items_per_tran, y=age_group, by=c("customer_id"="customer_id", "t_dat"="t_dat"))

str(items_per_tran)
```

```{r}
items_per_tran <- rename(items_per_tran, n_items = n)
...
```{r}
all_sample_trans_count <- merge(x=all_sample_trans, y=items_per_tran, by.x=c("t_dat", "customer_id", "age", "age_group"), by.y = c("t_dat", "customer_id", "age", "age_group"))
